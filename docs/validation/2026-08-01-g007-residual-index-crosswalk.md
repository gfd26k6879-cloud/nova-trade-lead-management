...
| M:next family | `ai_usage_events` | `idx_ai_usage_actor_created(actor_user_id, created_at DESC)` | actor totals; G-014/G-020 |
| M:next family | `ai_usage_events` | `idx_ai_usage_created(created_at DESC)` | time-window aggregate; G-017/G-020 |
...
| M:next family | `ai_usage_events` | `idx_ai_usage_model_created(model, created_at DESC)` | named platform/model aggregate; G-020 |

G-005 contributes zero residual indexes under this definition. The exact next
bounded work is the unnumbered read-only AI-usage query-history family of
`idx_ai_usage_actor_created`, `idx_ai_usage_created`, and
`idx_ai_usage_model_created`. This appendix does not open or invent a P20 card.
